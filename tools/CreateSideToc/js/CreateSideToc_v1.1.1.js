...
function createSideToc(source, target, install) {
    ...
    if (allowFold) {
        ...
        for (let i = 0; i < token.length; i++) {
            ...
            if (ul.children.length <= 0) {
                ...
                // 初始化折叠状态
                li.classList.add(defaultFold ? liFoldClassName : liUnfoldClassName);
                // 添加点击事件
                a.addEventListener('click', e => {
                    ...
                    let rangeRight;
                    if (before.boxSize === 'border-box') {
                        rangeRight = parseInt(before.width);
                    } else {
                        let {
                            width,
                            marginLeft: ml,
                        ...
                        let p = s => { return parseInt(s); }
                        rangeRight = p(ml) + p(mr) + p(pl) + p(pr) + p(bl) + p(br) + p(width) + rangeLeft;
                    }
                    // 是否点击伪元素范围内
                    if (e.offsetX < rangeLeft || rangeRight < e.offsetX) {
    ...
    if (viewFollow) {
        ...
        source.addEventListener('scroll', e => {
            for (let i = 0; i < token.length; i++) {
                let item = token[i];
                let top = item.header.getBoundingClientRect().top;
                if (top > source.offsetTop + ((source.clientHeight < 90) ? 30 : source.clientHeight / 3)) {
                    item = token[i ? i - 1 : 0];
                    // 删除上次阅读的样式
                    let beforeRead = document.getElementsByClassName(liCurrentReadClassName)[0];
                    if (beforeRead) { beforeRead.classList.remove(liCurrentReadClassName); }
                    // 修改正在阅读的目录样式
                    item.li.classList.add(liCurrentReadClassName);
                    item.li.scrollIntoView({ block: "center", behavior: 'smooth' });
                    break
                }
            }
        });
